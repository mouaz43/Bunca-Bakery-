// server.js — Bakeflow backend (UI-compatible, streamlined materials)
// Auth via ENV; Materials/Items/BOM/Plan; Tools; Users
...
app.use(session({
  ...
  cookie: { httpOnly: true, sameSite: 'lax', secure: NODE_ENV === 'production' ? 'auto' : false, maxAge: 1000*60*60*8 }
}));
...
const U = {
  ...
  pieces: { base: 'pcs', factor: 1 }, stk: { base: 'pcs', factor: 1 }, 'stück': { base: 'pcs', factor: 1 },
};
const normalizeUnit = (u) => { const k = String(u||'').trim().toLowerCase(); return U[k]?.base || (k || null); };
function packToBaseQty(pack_qty, pack_unit, base_unit) {
  ...
  if (m.base !== bu) return null; // families must match
...
app.post('/api/login', (req, res) => {
  ...
  if (ADMIN_EMAIL && ADMIN_PASSWORD && eqi(email, ADMIN_EMAIL) && pass === ADMIN_PASSWORD) {
    req.session.user = { email, role:'admin' }; return res.json({ ok:true });
  }
...
app.get('/api/materials', requireAuth, async (_req, res) => {
  const { rows } = await q(`SELECT code,name,base_unit,pack_qty,pack_unit,pack_price,price_per_unit,supplier_code,note FROM materials ORDER BY name`);
...
app.get('/api/materials/:code/history', requireAuth, async (req, res) => {
  ...
  const { rows } = await q(
    `SELECT price_per_unit, changed_at FROM material_price_history WHERE material_code=$1 ORDER BY changed_at DESC`, [code]
  );
  res.json({ ok:true, data: rows });
});

app.post('/api/materials', requireAuth, async (req, res) => {
  const { code, name, base_unit='g', pack_qty=null, pack_unit=null, pack_price=null, price_per_unit=null, supplier_code=null, note='' } = req.body || {};
  if (!code || !name) return res.status(400).json({ ok:false, error:'code_name_required' });

  const bu = normalizeUnit(base_unit || 'g');
  // If PPU not provided, derive it from pack fields
...
app.delete('/api/materials/:code', requireAuth, async (req, res) => {
  const code = req.params.code;
  // prevent FK break in bom (restrict)
  const used = await q(`SELECT 1 FROM bom WHERE material_code=$1 LIMIT 1`, [code]);
  if (used.rowCount) return res.status(400).json({ ok:false, error:'in_use_in_bom' });
  const r = await q(`DELETE FROM materials WHERE code=$1`, [code]);
  res.json({ ok:true, deleted: r.rowCount });
});

// Bulk price paste stays (optional for you)
...
app.get('/api/items/:code/scale', requireAuth, async (req, res) => {
  ...
  try { const r = await scaleRecipe(code, qty); res.json({ ok:true, data:r }); }
...
app.get('/api/plan/week', requireAuth, async (req, res) => {
  ...
  const end = fmtDate(plusDays(startDt, 7));
  const { rows } = await q(
...
app.post('/api/plan/calc', requireAuth, async (req, res) => {
  ...
  const arr = Array.from(need.values()).map(x => ({
    material_code:x.material_code, material_name:x.material_name, unit:x.unit,
    qty:Number(x.qty.toFixed(2)), price_per_unit:Number(x.price_per_unit.toFixed(6)), cost:Number(x.cost.toFixed(2))
  })).sort((a,b)=>a.material_name.localeCompare(b.material_name));
  res.json({ ok:true, data:{ lines:arr, total_cost:Number(totalCost.toFixed(2)) } });
});

/* ---------- Tools & Users (unchanged from previous answer) ---------- */
function toCSV(rows){ if(!rows||!rows.length) return ''; const keys=Object.keys(rows[0]); const esc=v=>v==null?'':(/[",\n]/.test(String(v))?`"${String(v).replace(/"/g,'""')}"`:String(v)); return [keys.join(','),...rows.map(r=>keys.map(k=>esc(r[k])).join(','))].join('\n'); }

app.get('/api/tools/export/:dataset', requireAuth, async (req, res) => {
  const ds = req.params.dataset; const fmt = (req.query.format||'json').toLowerCase(); let rows=[];
  ...
  else if (ds==='plan'){ const start=req.query.start||fmtDate(monday(new Date())); const end=fmtDate(plusDays(new Date(start),7)); rows=(await q(`SELECT * FROM production_plan WHERE day >= $1 AND day < $2 ORDER BY day,id`,[start,end])).rows; }
  else return res.status(400).json({ ok:false, error:'unknown_dataset' });

  if (fmt==='json'){ res.setHeader('Content-Disposition', `attachment; filename="${ds}.json"`); return res.json(rows); }
  if (fmt==='csv' || fmt==='xlsx'){ const csv=toCSV(rows); res.setHeader('Content-Type','text/csv; charset=utf-8'); res.setHeader('Content-Disposition',`attachment; filename="${ds}.csv"`); return res.send(csv); }
...
app.post('/api/tools/backup/restore', requireAuth, async (req, res) => {
  ...
  await q('BEGIN');
  try {
    await q('DELETE FROM bom'); await q('DELETE FROM production_plan'); await q('DELETE FROM items'); await q('DELETE FROM materials');
    for (const m of materials) await q(`INSERT INTO materials(code,name,base_unit,pack_qty,pack_unit,pack_price,price_per_unit,supplier_code,note) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
      [m.code,m.name,normalizeUnit(m.base_unit||'g'),m.pack_qty??null,m.pack_unit??null,m.pack_price??null,Number(m.price_per_unit||0),m.supplier_code||null,m.note||'']);
    for (const it of items) await q(`INSERT INTO items(code,name,category,yield_qty,yield_unit,note) VALUES($1,$2,$3,$4,$5,$6)`,
      [it.code,it.name,it.category||'',Number(it.yield_qty||1),it.yield_unit||'pcs',it.note||'']);
    for (const b of bom) await q(`INSERT INTO bom(product_code,material_code,qty,unit) VALUES($1,$2,$3,$4)`,
      [b.product_code,b.material_code,Number(b.qty),normalizeUnit(b.unit||'g')]);
    for (const p of plan) await q(`INSERT INTO production_plan(day,shop,start_time,end_time,product_code,qty,note) VALUES($1,$2,$3,$4,$5,$6,$7)`,
      [p.day||p.date,p.shop??null,p.start_time??null,p.end_time??null,p.product_code,Number(p.qty||p.planned_qty||0),p.note||'']);
    await q('COMMIT'); res.json({ ok:true });
  } catch(e){ await q('ROLLBACK'); res.status(400).json({ ok:false, error:e.message }); }
});

app.get('/api/users', requireAuth, async (_req, res) => { const { rows } = await q(`SELECT email, role FROM users ORDER BY email`); res.json({ ok:true, data:rows }); });
app.post('/api/users', requireAuth, async (req, res) => { const { email, role='viewer', password='' } = req.body||{}; if(!email) return res.status(400).json({ ok:false, error:'email_required' });
  await q(`INSERT INTO users(email,role,password) VALUES($1,$2,$3) ON CONFLICT (email) DO UPDATE SET role=EXCLUDED.role, password=EXCLUDED.password`, [email.trim(), role, password]); res.json({ ok:true }); });
app.delete('/api/users/:email', requireAuth, async (req, res) => { await q(`DELETE FROM users WHERE email=$1`, [req.params.email]); res.json({ ok:true }); });

/* ---------- Root & 404 ---------- */
app.get('/', (req, res) => authed(req) ? res.sendFile(path.join(__dirname,'public','dashboard.html')) : res.sendFile(path.join(__dirname,'public','login.html')));
app.use((req, res) => res.status(404).send('Not found'));

/* ---------- Boot ---------- */
(async () => { await ensureSchema(); app.listen(PORT, () => console.log(`Bakeflow running on :${PORT}`)); })();
