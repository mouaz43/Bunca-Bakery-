...
/* ---------------------------- Schema --------------------------------
   Idempotent & safe. Handles prior drift:
   - suppliers: adds contact/phone/email if missing
   - materials: (unit, price_per_unit, supplier_code, note)
   - items: recipes table
   - bom: ensures name + FKs (renames legacy bom_items -> bom, product_code/material_code)
   - production_plan
   - material_price_history
------------------------------------------------------------------------ */

async function ensureSchema() {
  const exec = (sql) => q(sql);
    ...
    await exec(`
      ...
      BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE table_name='materials' AND constraint_name='materials_supplier_fk') THEN
          ALTER TABLE materials DROP CONSTRAINT materials_supplier_fk;
        END IF;
      ...
      BEGIN
        ...
        END IF;
        IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE table_name='bom' AND constraint_name='bom_material_fk') THEN
...
function toBase(qty, unit) {
  ...
  const m = U[u];
  return m ? { qty: Number(qty) * m.factor, unit: m.base } : { qty: Number(qty), unit: u || unit };
}
  ...
  for (const s of suppliers) {
    await q(
      `INSERT INTO suppliers(code,name,contact,phone,email)
       ...
       SET name=EXCLUDED.name, contact=EXCLUDED.contact, phone=EXCLUDED.phone, email=EXCLUDED.email`,
      [s.code, s.name, s.contact || '', s.phone || '', s.email || '']
    );
  }

  for (const m of materials) {
    const unit = (m.unit || 'g').toLowerCase();
    const ppu = m.price_per_unit == null ? 0 : Number(m.price_per_unit);
    await q(
      ...
      [m.code, m.name, unit, ppu, m.supplier_code || null, m.note || '']
    );
  }
  ...
  for (const b of bom) {
    ...
    for (const ing of b.ingredients || []) {
      await q(
        ...
        [b.product_code, ing.material_code, Number(ing.qty), (ing.unit || 'g').toLowerCase()]
      );
    }
  }
...
async function scaleRecipe(productCode, targetQty) {
  ...
  const lines = await q(
    `SELECT b.material_code, b.qty, b.unit,
            m.unit AS m_unit, m.price_per_unit, m.name AS material_name
     FROM bom b
...
async function calcForPlanRows(rows) {
  ...
  for (const r of rows) {
    ...
    for (const l of one.lines) {
      ...
      const cur =
        need.get(key) ||
        { material_code: l.material_code, material_name: l.material_name, unit: l.unit, qty: 0, price_per_unit: l.price_per_unit, cost: 0 };
      cur.qty += l.qty;
      cur.cost += l.cost;
      need.set(key, cur);
    }
...
app.get('/api/materials', requireAuth, async (req, res) => {
  const { rows } = await q(
    `SELECT m.code, m.name, m.unit, m.price_per_unit, m.supplier_code, m.note,
...
app.post('/api/materials', requireAuth, async (req, res) => {
  ...
  await q(
    `INSERT INTO materials(code,name,unit,price_per_unit,supplier_code,note)
     ...
     SET name=EXCLUDED.name, unit=EXCLUDED.unit, price_per_unit=EXCLUDED.price_per_unit,
         supplier_code=EXCLUDED.supplier_code, note=EXCLUDED.note`,
    [code, name, unit.toLowerCase(), Number(price_per_unit), supplier_code, note]
  );
...
app.post('/api/items/:code/bom', requireAuth, async (req, res) => {
  ...
  for (const L of lines) {
    await q(
      `INSERT INTO bom(product_code,material_code,qty,unit) VALUES($1,$2,$3,$4)`,
      [code, L.material_code, Number(L.qty), (L.unit || 'g').toLowerCase()]
    );
  }
  res.json({ ok: true });
});
...
app.post('/api/plan/calc', requireAuth, async (req, res) => {
  ...
  if ((!list || !Array.isArray(list)) && req.body?.date) {
    const { rows } = await q(`SELECT product_code, qty FROM production_plan WHERE day=$1`, [
      req.body.date,
    ]);
...
async function start() {
  try {
    ...
    // optional: seed on first boot or when SEED_ON_START=true
    const c1 = await q(
      `SELECT
         (SELECT COUNT(*)::int FROM materials) AS m,
         (SELECT COUNT(*)::int FROM items)     AS i`
    );

    if (SEED_ON_START || (c1.rows[0].m === 0 && c1.rows[0].i === 0)) {
      console.log('[seed] applying (auto)…');
      try {
        await applySeed();
      } catch (e) {
