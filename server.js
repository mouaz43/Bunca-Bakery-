...
// --------------------- Seed ---------------------
function readSeed(name){ const p=path.join(__dirname,'seed',name); if(!fs.existsSync(p)) return null; return JSON.parse(fs.readFileSync(p,'utf8')); }
    ...
    await q(
      `INSERT INTO materials(code,name,base_unit,pack_qty,pack_unit,pack_price,price_per_unit,supplier_code,note)
       ...
       ON CONFLICT (code) DO UPDATE
         SET name=EXCLUDED.name, base_unit=EXCLUDED.base_unit, pack_qty=EXCLUDED.pack_qty, pack_unit=EXCLUDED.pack_unit,
             pack_price=EXCLUDED.pack_price, price_per_unit=EXCLUDED.price_per_unit, supplier_code=EXCLUDED.supplier_code, note=EXCLUDED.note`,
  ...
  for(const it of items){
    await q(
      `INSERT INTO items(code,name,category,yield_qty,yield_unit,note)
       VALUES($1,$2,$3,$4,$5,$6)
       ON CONFLICT (code) DO UPDATE
         SET name=EXCLUDED.name, category=EXCLUDED.category, yield_qty=EXCLUDED.yield_qty, yield_unit=EXCLUDED.yield_unit, note=EXCLUDED.note`,
...
app.post('/api/admin/seed/apply', requireAuth, async (req,res)=>{
  try{ const r=await applySeed(); res.json({ ok:true, counts:r }); }
...
app.get('/api/materials', requireAuth, async (req,res)=>{
  const { rows } = await q(`
    ...
    FROM materials m LEFT JOIN suppliers s ON s.code=m.supplier_code
    ORDER BY m.name`);
...
app.post('/api/materials', requireAuth, async (req,res)=>{
  ...
  await q(`INSERT INTO materials(code,name,base_unit,price_per_unit,supplier_code,note)
           VALUES($1,$2,$3,$4,$5,$6)
           ON CONFLICT (code) DO UPDATE
...
app.post('/api/materials/bulk-prices', requireAuth, async (req,res)=>{
  const text=String(req.body?.text||''); const lines=text.split(/\r?\n/).map(s=>s.trim()).filter(Boolean);
  let updated=0;
  for(const L of lines){
    const m=L.match(/^([^|]+)\|\s*([0-9]+(?:[.,][0-9]+)?)$/); if(!m) continue;
    const code=m[1].trim(); const val=Number(m[2].replace(',','.'));
    const prev=await q(`SELECT price_per_unit FROM materials WHERE code=$1`,[code]);
    ...
    const r=await q(`UPDATE materials SET price_per_unit=$1 WHERE code=$2`,[val,code]); updated+=r.rowCount;
  }
...
app.post('/api/items/:code/bom', requireAuth, async (req,res)=>{
  const { code }=req.params; const lines=Array.isArray(req.body?.lines)?req.body.lines:[];
...
app.get('/api/items/:code/scale', requireAuth, async (req,res)=>{
  const code=req.params.code; const qty=Number(req.query.qty||0);
...
app.get('/api/plan', requireAuth, async (req,res)=>{
  const { date }=req.query; if(!date) return res.json({ ok:true, data:[] });
  const { rows }=await q(`
...
app.post('/api/plan/save', requireAuth, async (req,res)=>{
  ...
  for(const r of rows){
    await q(`INSERT INTO production_plan(day,start_time,end_time,product_code,qty,note)
             VALUES($1,$2,$3,$4,$5,$6)`,
