// Bakeflow / BUNCA — Full Backend (stable sessions, complete CRUD, workflow calc)
// - Auth via ENV ADMIN_EMAIL / ADMIN_PASSWORD (email case-insensitive)
// - Postgres schema migrations are idempotent (safe to rerun)
// - Materials CRUD (+pack price -> €/base auto, price history, bulk update)
// - Items CRUD (+BOM CRUD, scaling & cost calc)
// - Plan CRUD (day/week), usage aggregation cost calc
// - Tools: import/export JSON/CSV, backup/restore JSON
// - Users list/upsert/delete (UI admin panel)
// - Static /public pages
// - Health & useful diagnostics
...
app.use(session({
  ...
  rolling: true,                       // extends cookie on activity (keeps you logged in)
  cookie: {
    httpOnly: true,
    sameSite: 'lax',
...
function packToBaseQty(pack_qty, pack_unit, base_unit) {
  ...
  if (m.base !== bu) return null; // unit families must match (g/kg vs ml/l vs pcs)
...
app.get('/api/materials/:code/history', requireAuth, async (req, res) => {
  const { rows } = await q(
    `SELECT price_per_unit, changed_at
     ...
     WHERE material_code=$1 ORDER BY changed_at DESC`, [req.params.code]
  );
  res.json({ ok: true, data: rows });
});

// Create/Update (UPSERT) — price_per_unit auto if omitted
...
// Bulk prices "CODE | 0.00123" per line
...
app.delete('/api/items/:code', requireAuth, async (req, res) => {
  ...
  const r = await q(`DELETE FROM items WHERE code=$1`, [code]);
  res.json({ ok: true, deleted: r.rowCount });
});
...
// Save a week (replace within range)
...
// Calculate usage/cost for arbitrary rows or for a specific date/week
app.post('/api/plan/calc', requireAuth, async (req, res) => {
  let list = req.body?.rows;

  // convenience: if date provided but no rows, use rows of that date
  ...
  if ((!list || !Array.isArray(list)) && req.body?.week_start) {
    ...
    const { rows } = await q(
      `SELECT product_code, qty FROM production_plan WHERE day >= $1 AND day < $2`, [s, e]
    );
    list = rows.map(r => ({ product_code: r.product_code, qty: Number(r.qty) }));
  }
...
/* ---------- Tools: import/export/backup/restore ---------- */
function toCSV(rows) {
...
app.get('/api/tools/export/:dataset', requireAuth, async (req, res) => {
  ...
  if (fmt === 'csv' || fmt === 'xlsx') { // xlsx falls back to csv
